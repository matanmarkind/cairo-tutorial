...
from starkware.cairo.common.dict import (
    dict_read, dict_write, dict_update, dict_squash, dict_new)
...
struct AmmState:
    # Dict which tracks the account states. Each element represents a
    # transaction: keyed by an account ID, with `prev_value/new_value` 
    # being the states before and after the transaction respectively.
    # {account_id : AccountChange}
    member account_dict_start : DictAccess*
    member account_dict_end : DictAccess*

    # The amount of tokens currently in the AMM. Must be in the range
    # [0, MAX_BALANCE].
    member token_a_balance : felt
    member token_b_balance : felt
end
...
# Returns a hash committing to the account's state using the following formula:
...
# For each entry in the input dict (represented by dict_start and dict_end)
# write an entry to the ouput dict (represented by hash_dict_start and
# hash_dict_end) after applying hash_account on prev_value and new_value and
...
func hash_dict_values{pedersen_ptr : HashBuiltin*}(
    ...
    # ??? - Does this mutate `hash_dict_start`? Otherwise when does it get
    # updated?
...
func swap{range_check_ptr}(
    ...
    # Here you should berify the user has signed on a message specifying that
    # they would like to sell 'a' tokens of token_a. You should use the public
    ...
    %{
        print(
            ...
            f'gave {ids.a} tokens of type `a` and received {ids.b} '
            f'tokens of type `b`.')
    %}
...
func transaction_loop{range_check_ptr}(
    ...
    # ??? - When do I put parens around a return value?
...
func modify_account{range_check_ptr}(
    ...
    # Define a reference to state.account_dict_end so that we can use it as an
    # implicit argument to the dict functions.
    let account_dict_end = state.account_dict_end

    # Retrieve the pointer to the current state of the account.
    # ??? - Why are we passing in end? I would have expected to pass in start
    # and size.
    ...
    # ??? - Why are we passing in end? I would have expected to pass in start
    # and size. How do we pass in info about the new_value size. How does it
    # know how much to copy? If we are just copying in the public_key, why not
    # just write that?
    ...
    local new_state : AmmState
    assert new_state.account_dict_start = state.account_dict_start
    # ??? - So now `account_dict_end != state.account_dict_end`. When
    # was it updated? Implicit params can be updated?
...
# Hint argument: initial_account_dict should be a dictionary from account_id
# to an address in amemory of the Account struct.
func compute_merkle_roots{pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    # ??? - is this needed to pass it implicitly to other functions? It
    # just seems confusing to have this here. I think it would be better to
    # pass it explicitly.
    ...
    %{
        ...
        initial_dict = {}  # Implicit param to `dict_new`.
        for account_id, account in initial_account_dict.items():
            ...
            initial_dict[account_id] = pedersen_hash(
                ...
                token_b_balance)
    %}
    # ??? - Does this create a dict with all the values from initial_dict? If
    # so, why not pass back the size/end?
...
func get_account_dict() -> (account_dict : DictAccess*):
    ...
    %{
        ...
        # Save a copy of the initial account dict for compute_merkle_roots.
        # ??? - So we need to save this for a function that isn't even
        # called from the local function :(
...
func main{
    ...
    %{
        # Initialize the balances using a hint. Later we will output them to
        # the output struct, which will allow the verifier to check that they
        # are indeed valid.
